...
class dataset:
    def __init__(self, dataset_path: str, display_info: bool = False):
...
def colorize(image_pathstr, data_pathstr, save_to = None):
    ...
    visualize_colorized_coefficients(coef_map, tracer_names=tracer_dataset.tracer_list)

def visualize_colorized_coefficients(coef_map, tracer_names=None):
    K, H, W = coef_map.shape
    colormaps = ['Reds', 'Greens', 'Blues', 'Purples', 'Oranges', 'Greys']
    ...
    for k in range(K):
        coef = coef_map[k]
        norm = Normalize(vmin=0, vmax=np.max(coef))
        cmap = cm.get_cmap(colormaps[k % len(colormaps)])
        rgba_img = cmap(norm(coef))

        axes[k].imshow(rgba_img)
        if tracer_names:
            axes[k].set_title(tracer_names[k])
        else:
            axes[k].set_title(f"Tracer {k}")
        axes[k].axis('off')
